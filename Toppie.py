...
This is the main file for Toppie. When executed, a real time plot of the density distribution, 
compliance and density error is displayed. The user is free to modify the code to test differnt
problems, material types, solver types, etc. 


------------------------------------------------------------------------------------------------


In the "__name__ == '__main__'" section at the end of the file, a user can specify the following:

material_type: 
    "I" for Isotropic and "O" for Orthotropic

problem_type : 
    "h" for a half MBB beam
    "f" for a full MBB beam
    "c" for a cantilever beam with a force applied downwards halfway up the right edge  
    "c2" for a cantilever beam with a force applied downwards at the top right node

Isotropic Material Properties:
    E : Young's Modulus (normalised values) i.e. specifify 1 for 1 GPa and not 1e9 for 1 GPa
    v : Poisson's Ratio i.e. 0.3, 0.27 etc.           
...
Orthotropic Material Properties (Glass/epoxy) 
    ...
    v12   : Poisson's Ratio
    G12   : Shear Modulus
    theta : fibre angle (degrees) relative to the global x-axis (IN DEGREES)

Problem Parameters:
    X     : number of elements in the x-direction i.e. 60 for 60 elements in the x-direction
    Y     : number of elements in the y-direction i.e. 20 for 20 elements in the y-direction
    f     : volume fraction i.e. 0.5 for 50% volume fraction
    P     : penalisation factor (typically 3)
    rmin  : filter radius (use 2.5). Reducing filter radius will improve resolution and
            the resulting density distribution will be more well defined and less blurred,
            however this is at the expense of numerical stability. Low values for rmin,
            lead to checkerboarding and numerical instabilities.
    move  : move limit (typically 0.2)
    force : point load magnitude (N)

    max_iterations : maximum number of iterations for the optimisation algorithm 
    
solver_type (no need for modification):

    1. "cg" for Conjugate Gradient
    2. "spsolve" for sparse solve
    3. "gmres" for Generalised Minimal Residual Method (not recommend, very slow)
    4. "bicgstab" for BiConjugate Gradient Stabilised Method
    5. "cvxopt" for LU decomposition using the CVXOPT library
    6. "lgmres" for Generalised Minimal Residual Method 
    7. "qmr" for Quasi-Minimal Residual Method
    8. "SuperLU" for LU decomposition
    9. "UMFPACK" for LU decomposition alternative

    fastest solver is "cvxopt"
...
class Visualization:
    ...
    def __init__(self):
        ...
        self.fig, self.axs = plt.subplots(1, 3, figsize=(18, 5))
        self.compliance_values  = []
        self.iterations         = []
        self.density_errors     = []

        self.problem_dict  = {"h": "Half MBB", "f": "Full MBB", "c": "Cantilever (Type 1)", "c2": "Cantilever (Type 2)"}
        self.material_dict = {"O": "Orthotropic (θ = {}°)", "I": "Isotropic"}

        self.compliance_line,    = self.axs[1].plot([], [], label='Compliance')
        self.density_error_line, = self.axs[2].plot([], [], color='red', label='Density Error')
        ...
        self.axs[1].set_ylabel('Compliance')
        self.axs[1].set_title('Compliance', fontweight='bold', loc='center', pad=-15, color='black')
        
        self.axs[2].set_xlabel('Iteration')
        self.axs[2].set_ylabel('Density Error')
        self.axs[2].set_title('Maximum Elemental Density Change per Iteration', fontweight='bold', loc='center', pad=-15, color='black')

    def display_results(self, loop, c, x, X, Y, error, problem_type, material_type, theta):
        
        heading = f"Problem Type: {self.problem_dict[problem_type]}, Material: {self.material_dict[material_type].format(theta)}"
        
        self.compliance_values.append(c)
        self.iterations.append(loop)
        self.density_errors.append(error)

        self.axs[0].clear()
        self.axs[0].imshow(-x, cmap='gray', origin='upper', extent=[0, X, 0, Y])
        self.axs[0].set_title(heading, fontweight='bold')
        self.axs[0].axis('on')
        self.axs[0].set_xlim(0, X)
        self.axs[0].set_ylim(0, Y)

        self.compliance_line.set_ydata(self.compliance_values)
        self.compliance_line.set_xdata(self.iterations)
        
        self.density_error_line.set_ydata(self.density_errors)
        self.density_error_line.set_xdata(self.iterations)
...
def main(X, Y, f, P, rmin, material_type, E, v, E1, E2, v12, G12, theta, problem_type, move, solver_type, force, max_iterations):
    ...
    while error > 0.01 and current_iteration < max_iterations:
        ...
        inst_visualization.display_results(loop, C, x, X, Y, error, problem_type, material_type, theta)
