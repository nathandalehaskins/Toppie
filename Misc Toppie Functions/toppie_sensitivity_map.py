...
Here Toppie.py has been modified to plot the sensitivity map alongside the density distribution.
This complements Section 4.6.1 Sensitivity Analysis in the main report.

NOTE:
To save snapshots of the density distribution and sensitivity map, uncomment the lines 
624 - 625. This will save three images to the current directory. The images will contain
the senstivity map and associated density distribution for the 10th, 20th and 100th iteration.
The reader is free to modify these lines to save snapshots at different iterations.
...
class Visualization:
    ...
    def __init__(self, X, Y):
        ...
        self.im_topology = self.axs[0].imshow(np.zeros((X, Y)), cmap='gray', origin='upper', extent=[0, X, 0, Y], aspect='equal')
        self.axs[0].set_title('Topology')
        self.axs[0].axis('off')  

        self.im_sensitivity = self.axs[1].imshow(np.zeros((X, Y)), cmap='viridis', origin='upper', extent=[0, X, 0, Y], aspect='equal')
        colorbar = plt.colorbar(self.im_sensitivity, ax=self.axs[1], shrink=0.4)  
        self.axs[1].set_title('Sensitivity Map',fontweight='bold')
        colorbar.set_label('Sensitivity',fontweight='bold')
        self.axs[1].axis('off') 

    def display_results(self, loop, c, x, X, Y, error, dc, problem_type, material_type, theta):
        heading = "Topology"
        
        self.axs[0].clear()
        self.axs[0].imshow(-x, cmap='gray', origin='upper', extent=[0, X, 0, Y], aspect='equal')
        self.axs[0].set_title(heading, fontweight='bold')
        self.axs[0].axis('off')  
        self.axs[0].set_xlim(0, X)
        self.axs[0].set_ylim(0, Y)

        self.im_sensitivity.set_array(dc)
        
        # Capture snapshots at specific iterations
        # if loop in [10, 20, 100]:
        #     plt.savefig(f"snapshot_at_iteration_{loop}.png")
...
def main(X, Y, f, P, rmin, material_type, E, v, E1, E2, v12, G12, theta, problem_type, move, solver_type, force, max_iterations):
    ...
    inst_visualization = Visualization(X,Y)
    ...
    while error > 0.01 and current_iteration < max_iterations:
        ...
        C, dc_1 = inst_optimisation.comp(x, U, ke, P, edofMat)  
        dc    = inst_optimisation.filt(x, rmin, dc_1)
        x     = inst_optimisation.OC(X, Y, x, f, dc)
        error = np.max(np.abs(x - xold))   

        inst_visualization.display_results(loop, C, x, X, Y, error, dc_1, problem_type, material_type, theta)
...
if __name__ == '__main__':
    ...
    X    = 240      # number of elements in the x-direction
    Y    = 80       # number of elements in the y-direction
